...
# 使用SiameseMNISTDataset
transform = transforms.Compose([transforms.ToTensor()])
mnist_train = MNIST(root=root, train=True, download=True, transform=transform)
mnist_test = MNIST(root=root, train=False, download=True, transform=transform)

siamese_train_dataset = SiameseMNISTDataset(mnist_train)
siamese_test_dataset = SiameseMNISTDataset(mnist_test)
...
for epoch in range(num_epochs):
    ...
    accuracy_train = correct_train / len(siamese_train_dataset)
    ...
    accuracy_test = correct_test / len(siamese_test_dataset)
